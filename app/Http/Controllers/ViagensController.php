...
class ViagensController extends Controller
{
    ...
    public function getWeatherDataForDestination(Viagens $viagem, Destinos $destino)
    {
        ...
        try {
            ...
            $hoje = Carbon::today();
                ...
                // Permitir exibir previsão até 7 dias antes da chegada do destino
                $limiteExibicao = $dataInicio->copy()->subDays(7)->startOfDay();
                if ($hoje->lt($limiteExibicao)) {
                    // Muito antes do destino — previsão indisponível
                    return response()->json([
                        'success' => true,
                        'data' => null,
                        'message' => 'A previsão do tempo estará disponível a partir de ' . $limiteExibicao->format('Y-m-d') . '.'
                    ]);
                }
    ...
    public function showApi($id) {
        try{
            ...
            */
            ...
            // Busca o clima na Open-Meteo — exibe até 7 dias antes do início da viagem
            $coordenadas = $this->getLatLngFromAddress($nomeDestino);
            $dataInicio = Carbon::parse($viagem->data_inicio_viagem)->startOfDay();
            $dataFim = Carbon::parse($viagem->data_final_viagem)->endOfDay();
            $hoje = Carbon::today();
            $limiteExibicao = $dataInicio->copy()->subDays(7)->startOfDay();

            $clima = null;
            // Se hoje for antes do limite (mais de 7 dias antes) ou após o fim da viagem, não exibir
            if ($hoje->lt($limiteExibicao) || $hoje->gt($dataFim)) {
                $clima = null;
            } else {
                if ($coordenadas) {
                    ...
                    $weatherUrl = "https://api.open-meteo.com/v1/forecast?latitude={$latitude}&longitude={$longitude}&daily=temperature_2m_max,temperature_2m_min,wind_speed_10m_max,precipitation_sum,rain_sum,precipitation_probability_max&start_date={$start}&end_date={$end}";
                    try {
                        $weatherResponse = @file_get_contents($weatherUrl);
                        $clima = $weatherResponse ? json_decode($weatherResponse, true) : null;
                    } catch (\Exception $e) {
                        Log::warning('Erro ao buscar clima (método antigo)', ['error' => $e->getMessage()]);
                        $clima = null;
                    }
                } else {
                    $clima = null;
    ...
    /**
     * Retorna as datas (início/fim) da viagem atualmente selecionada na sessão.
     * Usado como fallback pelo modal IA quando as variáveis window.* não existem.
     */
    public function currentTrip(Request $request)
    {
        // Allow an explicit trip_id param, fallback to session('trip_id')
        $tripId = $request->input('trip_id', session('trip_id'));
        if (!$tripId) {
            return response()->json(['success' => false, 'message' => 'Nenhuma viagem selecionada'], 204);
        }

        try {
            $viagem = Viagens::find($tripId);
            if (!$viagem) {
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'data_fim' => $viagem->data_final_viagem,
                    'id' => $viagem->pk_id_viagem,
                    'nome' => $viagem->nome_viagem ?? null
                ]
            ]);
        } catch (\Exception $e) {
            \Log::error('Erro ao obter viagem atual via API', ['error' => $e->getMessage()]);
