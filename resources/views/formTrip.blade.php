        ...
        <div class="w-full max-w-4xl mx-auto mt-12">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-10 mb-10 animate-fade-in">
                <form id="multiStepForm" method="POST" action="{{ route('formTrip.store') }}">
                    @csrf
                    <input type="hidden" name="seguroSelecionadoId" id="seguroSelecionadoId" value="">
                    <!-- Passo 1 -->
                    @include('trip.step1')
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Ao selecionar um seguro, salva o ID retornado do backend
    document.addEventListener('click', function(e) {
        let card = e.target.closest('.seguro-card, .insurance-card');
        if (card && card.classList.contains('seguro-card')) {
            try {
                ...
                sessionStorage.setItem('selectedSeguroName', fullName.trim());
                // Salva no backend e armazena o ID retornado
                seguroData.site = fullName.trim();
                fetch("/trip/salvar-seguro", {
                    method: "POST",
                    headers: {
                ...
                .then(data => {
                    if (data.seguro_id) {
                        sessionStorage.setItem('selectedSeguroId', data.seguro_id);
                        document.getElementById('seguroSelecionadoId').value = data.seguro_id;
                    }
                });
            } catch {}
        }
    ...
    // Ao submeter o formulário, envia o seguroSelecionadoId
    document.getElementById('multiStepForm').addEventListener('submit', function (e) {
        var seguroId = sessionStorage.getItem('selectedSeguroId');
        if (seguroId) {
            document.getElementById('seguroSelecionadoId').value = seguroId;
        }
    });
});
</script>