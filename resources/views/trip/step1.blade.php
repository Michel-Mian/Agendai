...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDateValidation(destinoIndex) {
        ...
        dataInicioInput.addEventListener('change', function() {
            // Atualizar data mínima da data de fim
            if (this.value) {
                dataFimInput.setAttribute('min', this.value);
                
                // Se a data de fim já estiver preenchida e for menor que a nova data de início, limpar
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Definir data mínima como hoje para o primeiro destino
        const today = new Date().toISOString().split('T')[0];
        const firstDataInicio = document.getElementById('destino_data_inicio_0');
        if (firstDataInicio) {
            firstDataInicio.setAttribute('min', today);
        }
