<div class="bg-white rounded-lg shadow-md px-4 py-4 md:px-8 md:py-6 mb-10">
    <div class="flex flex-col md:flex-row items-center md:items-stretch gap-4 md:gap-0 overflow-x-auto">
        <!-- Companhia -->
        <div class="flex items-center w-full md:w-48 min-w-0 mb-4 md:mb-0">
        ...
        <!-- Horário de saída -->
        <div class="flex flex-col items-center w-full md:w-32 min-w-0 mb-4 md:mb-0">
        ...
        <!-- Preço e Seleção -->
        <div class="flex flex-col items-end w-full md:items-end md:w-auto flex-1 min-w-0">
    ...
    <button class="ml-auto text-blue-600 font-semibold flex items-center hover:underline ver-detalhes-btn mt-4 md:mt-0" data-target="detalhes-{{ $index }}">
        Ver Detalhes <i class="fa-solid fa-chevron-right ml-1 text-xs"></i>
    </button>
    <div id="detalhes-{{ $index }}" class="detalhes-viagem hidden px-2 py-6 md:px-8 bg-gray-50 rounded-xl shadow-inner mt-6 transition-all duration-300">
        <h3 class="text-xl md:text-2xl font-bold mb-6 text-blue-700 flex items-center gap-2">
            ...
            <div class="space-y-6">
                <div>
                    <div class="flex flex-col sm:flex-row sm:items-center mb-4 gap-2">
                        ...
                        <span class="sm:ml-2">{{ $flight['flights'][0]['departure_airport']['name'] ?? '--' }} ({{ $flight['flights'][0]['departure_airport']['id'] ?? '--' }})</span>
                        <span class="sm:ml-4 text-gray-500">{{ isset($flight['flights'][0]['departure_airport']['time']) ? \Carbon\Carbon::parse($flight['flights'][0]['departure_airport']['time'])->format('d/m/Y H:i') : '--' }}</span>
                    </div>
                    <div class="flex flex-col sm:flex-row sm:items-center mb-2 gap-2">
                        ...
                        <span class="sm:ml-2">{{ $flight['flights'][count($flight['flights'])-1]['arrival_airport']['name'] ?? '--' }} ({{ $flight['flights'][count($flight['flights'])-1]['arrival_airport']['id'] ?? '--' }})</span>
                        <span class="sm:ml-4 text-gray-500">{{ isset($flight['flights'][count($flight['flights'])-1]['arrival_airport']['time']) ? \Carbon\Carbon::parse($flight['flights'][count($flight['flights'])-1]['arrival_airport']['time'])->format('d/m/Y H:i') : '--' }}</span>
                    </div>
                </div>
                <div>
