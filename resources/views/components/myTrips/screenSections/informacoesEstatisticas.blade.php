    ...
    const WeatherModule = (function () {
        ...
        async function load(tripId, destinoId) {
            ...
            try {
                ...
                // Caso a API retorne sucesso + dados válidos
                ...
                // Caso a API retorne sucesso mas sem dados (ex.: previsao indisponivel antes/depois do destino)
                if (data?.success && (data.data === null || data.data === undefined)) {
                    const content = document.getElementById('weather-content-stats');
                    if (content) {
                        const message = (data.message && typeof data.message === 'string') ? data.message : 'Dados de clima indisponíveis para este destino.';
                        content.innerHTML = `<div class="text-center py-8"><p class="text-gray-600">${message}</p></div>`;
                    }
                    UIState.set('weather', 'content');
                    return;
                }
