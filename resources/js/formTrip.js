...
// REMOVIDO: O código de seleção de preferências agora está no step3.blade.php
// usando event delegation para funcionar corretamente mesmo quando o passo é carregado dinamicamente
...
(function() {
    ...
    function coletarDadosViagem() {
        ...
        if (origemInput) {
            ...
            console.log('📍 Origem Place ID:', dadosViagem.origem_place_id);
        }
        ...
        destinoInputs.forEach((input, index) => {
            ...
            if (placeId && input.value.trim()) {
                ...
                console.log(`📍 Destino ${index + 1} Place ID:`, placeId);
            }
        });
        ...
        if (autonomiaInput && autonomiaInput.value) {
            ...
            console.log('⛽ Autonomia:', dadosViagem.autonomia, 'km/L');
        }
        ...
        if (tipoCombustivelSelect && tipoCombustivelSelect.value) {
            ...
            console.log('⛽ Tipo de combustível:', dadosViagem.tipo_combustivel);
        }
        ...
        if (precoCombustivelInput && precoCombustivelInput.value) {
            ...
            console.log('💰 Preço do combustível informado:', dadosViagem.preco_combustivel);
        } else {
            // Usar preço médio
            const precosMedios = {
                'gasolina': 5.89,
            ...
            console.log('💰 Preço do combustível (médio):', dadosViagem.preco_combustivel);
        }

        console.log('✅ Dados coletados:', dadosViagem);
        return dadosViagem;
    }
    ...
    async function calcularRota() {
        try {
            ...
            if (!dados.origem_place_id || dados.destinos_place_ids.length === 0) {
                console.warn('⚠️ Dados incompletos para calcular rota:', {
                    origem: dados.origem_place_id,
                    destinos_count: dados.destinos_place_ids.length
                });
                return null;
            }

            if (!dados.autonomia || dados.autonomia <= 0) {
                console.warn('⚠️ Autonomia não definida ou inválida:', dados.autonomia);
                return null;
            }

            console.log('📡 Enviando requisição para calcular rota...');
            ...
            console.log('✅ Rota calculada com sucesso:', data);
            ...
            console.log('💰 Cálculo de custos:', {
                ...
                litros: resultado.litros_necessarios,
                custo_combustivel: resultado.custo_combustivel,
                pedagio: resultado.pedagio_estimado,
    ...
    function setupStep7Listener() {
        console.log('⚙️ Configurando listener do Step 7...');
        
        const form = document.getElementById('multiStepForm');
        if (!form) {
            console.warn('⚠️ Formulário multi-step não encontrado');
            return;
        }
            ...
            for (const mutation of mutations) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    ...
                    if (target.classList.contains('form-step') && 
                        ...
                        console.log('🎯 Step 7 ativado, verificando se é carro próprio...');
                        
                        // Verificar se é carro próprio
                        const dados = coletarDadosViagem();
                        
                        console.log('🔍 Meio de locomoção detectado:', dados.meio_locomocao);
                        
                        if (dados.meio_locomocao === 'Carro (próprio)') {
                            console.log('🚗 Carro próprio detectado! Iniciando cálculo de rota...');
                            
                            // Calcular rota
                            const resultado = await calcularRota();
                            if (resultado && resultado.success) {
                                console.log('✅ Rota calculada, atualizando interface...');
                                atualizarStep7ComCalculos(resultado);
                            } else {
                                console.warn('⚠️ Falha ao calcular rota:', resultado);
                            }
                        } else {
                            console.log('ℹ️ Não é carro próprio, pulando cálculo de rota');
                        }
                    }
                }
            }
        ...
        console.log('✅ Listener do Step 7 configurado com sucesso');
    }
